...
from utils import to_multitask, create_global_data_bow, filter_run, build_record
import re
import os
import sys
import json
...
def main():
    ...
    parser.add_argument('-tr', '--train_assessed_relevant', required=True)
    parser.add_argument('-t', '--test_assessed_relevant', required=True)
    ...
    start = time.time()
    x_train_r, y_train_r, cxt_train_r = create_global_data_bow(args.train_assessed_relevant)
    x_test_r, y_test_r, cxt_test_r = create_global_data_bow(args.test_assessed_relevant)
    elapsed = time.time() - start
    print 'dataset read, took %s' % elapsed

    start = time.time()
    print 'to multitask x_train_r...'
    x_train_a_r_multitask = to_multitask(x_train_r, cxt_train_r, idxs_entities)
    elapsed = time.time() - start
    ...
    clf_uv = clf_uv.fit(x_train_a_r_multitask, y_train_r)
    elapsed = time.time() - start
