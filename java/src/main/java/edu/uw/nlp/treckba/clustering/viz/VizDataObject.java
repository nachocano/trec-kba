...
public class VizDataObject {
	...
	public void updateCluster(final Cluster c) {
		cluster.setCluster(c.meanNormalized());
	}

	public void updateClustersAndStalenesses(final List<Cluster> clusters,
			final Cluster c) {
		final List<float[]> clustersEmbeddings = new LinkedList<>();
		final List<Lambda> stalenessEmbeddings = new LinkedList<>();
		for (final Cluster clu : clusters) {
			if (c.equals(clu)) {
				System.out.println("same cluster, discarding");
			} else {
				clustersEmbeddings.add(clu.meanNormalized());
				stalenessEmbeddings.add(new Lambda(clu.getLambdaDecrease(), clu
						.getLambdaIncrease()));
			}
		}
	}
